...
Data_generation <- function(Cluster, Gamma11, T00, Gamma00, Gamma01, Gamma10, ClusterSize) {
  ...
  lv1_2 <- matrix(NA, nrow = Cluster * ClusterSize, ncol = 7)  # Increase the number of columns to accommodate Y_ceiling
  case <- 0
  for (j in 1:Cluster) {
    for (i in 1:ClusterSize) {
      ...
      Y_ceiling <- ifelse(Y <= 10, Y, 10)  # Add Y_ceiling calculation based on the condition
      lv1_2[case, ] <- c(j, w[j], x[case], u0[j], lamda, Y, Y_ceiling)  # Include Y_ceiling in the matrix
    }
  }
  data_1 <- as.data.frame(lv1_2)
  names(data_1) <- c("Cluster_ID", "w", "x", "u0", "lamda", "Y", "Y_ceiling")  # Update column names
  return(data_1)
}
...
for (i in 1:nrow(Conditions)) {
  ...
  for (J in 1:reps) {
    error1 <- try (
      model <- glmer(Y_ceiling ~ 1 + x + w + w * x + (1 | Cluster_ID),
                     data = Data[[i]][[J]],
                     family = poisson),
      silent = TRUE
    )
...
results_1 <- t(rbind(Conditions$Cluster, Conditions$Gamma11, Conditions$T00,Conditions$Gamma00, Conditions$Gamma10, Conditions$ClusterSize,
...
write.csv(results_1, file = "Results.csv",row.names=F)
